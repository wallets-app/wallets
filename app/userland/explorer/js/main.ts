...
export class ExplorerApp extends LitElement {
  ...
  render () {
    return html`
      <link rel="stylesheet" href="wallets://explorer/css/font-awesome.css">
      <div
        class=${classMap({
          layout: true,
  ...
  async onNewFile (e) {
    ...
    if (filename) {
      ...
      window.open(`wallets://editor/?url=${url}`)
    }
  }
  ...
  async onShowMenu (e, useAppMenuAlways = false) {
    ...
    if (!useAppMenuAlways && typeof beaker !== 'undefined' && typeof beaker.browser !== 'undefined') {
      ...
      return contextMenu.create({
        ...
        roomy: false,
        noBorders: true,
        fontAwesomeCSSUrl: 'wallets://explorer/css/font-awesome.css',
        style: `padding: 4px 0`,
  ...
  async doDiff (base) {
    if (this.attachedMode) beaker.browser.gotoUrl(`wallets://diff/?base=${base}`)
    else window.open(`wallets://diff/?base=${base}`)
  }
